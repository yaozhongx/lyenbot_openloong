...
读取 `codex_notes/TODO.md` 后，必须先确认相关任务的状态和已有验证证据：

- 状态为“已完成”的任务不得重复实现；
- 优先依据 `TODO.md`、`LYENBOT_ADAPTATION.md`、`DEBUG_LOG.md` 和 `CHANGELOG.md` 中已有的验证证据；
- 不得仅为再次确认结论而重复执行已经通过的修改或测试；
- 只有在用户明确要求、相关代码或模型发生变化、已有证据缺失或矛盾、或者发现回归风险时，才重新验证；
- 重新验证前必须说明原因、验证范围以及是否可能影响已有结论；
- 重新验证本身不自动改变原任务状态，只有新证据足以推翻原结论时，才同步更新相关工程文档。
